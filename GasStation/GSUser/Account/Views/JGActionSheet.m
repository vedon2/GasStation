...
#ifndef ___IPHONE_8_0
#define ___IPHONE_8_0 80000
#endif

#ifndef kBaseSDKiOS8
#define kBaseSDKiOS8 (__IPHONE_OS_VERSION_MAX_ALLOWED >= ___IPHONE_8_0)
#endif
    ...
    if (self) {
        ...
        if (buttonTitles.count) {
            ...
            for (int i = 0 ;i < [buttonTitles count];i++) {
                NSString *str  = [buttonTitles objectAtIndex:i];
                LZActionButton *b = [self makeButtonWithTitle:str style:buttonStyle];
                b.tag = index;
                [self addSubview:b];
                if (i != [buttonTitles count]-1 && [buttonTitles count] > 1) {
                    UIImageView *bottomLine = [[UIImageView alloc]init];
                    [bottomLine setBackgroundColor:GrayColor];
                    bottomLine.alpha = 0.6;
                    b.bottomImageView = bottomLine;
                    [self addSubview:b.bottomImageView];
                }
...
- (LZActionButton *)makeButtonWithTitle:(NSString *)title style:(JGActionSheetButtonStyle)style {
    LZActionButton *b = [LZActionButton buttonWithType:UIButtonTypeCustom];
...
#pragma clang diagnostic pop
    ...
    for (LZActionButton *button in self.buttons) {
        height += spacing;
        
        button.frame = (CGRect){{spacing, height}, {width-spacing*2.0f, buttonHeight}};
        if (button.bottomImageView) {
             button.bottomImageView.frame = CGRectMake(button.frame.origin.x,button.frame.origin.y + button.frame.size.height - 1, button.frame.size.width, 1);
        }
...
- (void)moveToPoint:(CGPoint)point arrowDirection:(JGActionSheetArrowDirection)arrowDirection animated:(BOOL)animated {
    ...
    void (^changes)(void) = ^{
        ...
        _scrollViewHost.layer.shadowColor = [UIColor whiteColor].CGColor;
